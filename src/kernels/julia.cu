...
__global__ void julia_kernel(
    ...
    color_pixel_julia(iter, maxIter, zx, zy, &pixels[idx], theme);
}
...
void julia_cuda(
    ...
    julia_kernel<<<grid, block>>>(pixels_d, width, height, centerX, centerY, scale, maxIter, juliaRe, juliaIm, theme);
